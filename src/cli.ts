...
program
  .name('gitdup')
  .description('Duplicate the current project directory, reset git changes, and optionally checkout a PR/branch.')
  .argument('[dest]', 'Destination directory (must not exist or be empty). Defaults to <cwd>-dup or -dup-N')
  .option('-b, --branch <name>', 'Branch name to checkout in the duplicate')
  .option('-p, --pr <number>', 'PR number to fetch and checkout (GitHub remotes)', (v) => Number(v))
  .option('-r, --remote <name>', 'Remote to use for PR fetch (default: origin)', 'origin')
  .option('--clean', 'Also remove untracked files (keeps ignored like .env)', false)
  .option('-v, --verbose', 'Verbose output for git operations', false)
  .option('--no-install', 'Skip package manager install in Node projects')
  .action(async (destArg: string | undefined, opts: any) => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.log(`${indent}${colors.cyan('source:')} ${linkifyPath(source)}`);
      if (destAbs) {
        // eslint-disable-next-line no-console
        console.log(`${indent}${colors.cyan('target:')} ${linkifyPath(destAbs)}`);
      }

      let currentStep = 'Starting';
      const totalSteps = 2 + (opts.clean ? 1 : 0) + ((opts.branch || typeof opts.pr === 'number') ? 1 : 0) + (willInstall ? 1 : 0);
      ...
      const updateFrom = (evt: GitDupProgressEvent) => {
        switch (evt.type) {
          ...
          case 'git:branch:start':
            spinner.text(`${stepLabel(totalSteps)} Checkout branch ${colors.bold(evt.branch)}`);
            break;
          case 'git:branch:done':
            spinner.text(`${stepLabel(totalSteps)} Checked out ${colors.bold(evt.branch)}`);
            break;
          case 'git:pr:start':
            spinner.text(
          ...
          case 'node:install:start':
            spinner.text(`${stepLabel(totalSteps)} Installing dependencies (${colors.bold(evt.manager)})`);
            break;
          case 'node:install:done':
            spinner.text(`${stepLabel(totalSteps)} Install complete (${colors.bold(evt.manager)})`);
            break;
          case 'node:install:skip':
            // Informative, but don't change step label
            break;
          case 'done':
            spinner.succeed(`${colors.green('Done')} ${colors.dim('— duplicate ready')}`);
            break;
        }
      };
      ...
      const res = await gitdup({
        dest: destArg,
        branch: opts.branch,
        pr: typeof opts.pr === 'number' && !Number.isNaN(opts.pr) ? opts.pr : undefined,
        remote: opts.remote,
        clean: !!opts.clean,
        verbose,
      ...
      summary.push(`${indent}${colors.green('duplicated:')} ${linkifyPath(res.dest)}`);
      if (res.checkedOut) summary.push(`${indent}${colors.green('checked out:')} ${colors.bold(res.checkedOut)}`);
      // eslint-disable-next-line no-console
      console.log(summary.join('\n'));
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      try { spinner.fail(colors.red('Failed')); } catch {}
      // eslint-disable-next-line no-console
